...
class StockApp(tk.Tk):
    ...
    def create_initial_frame(self):
        ...
        tk.Radiobutton(home_frame, text="Default Credentials", variable=self.credential_type, 
                        value="default",font=('Segoe UI',12), command=self.toggle_credentials_fields).grid(row=0, column=0, padx=5, pady=5)
        tk.Radiobutton(home_frame, text="Custom Credentials", variable=self.credential_type,
                value="custom",font=('Segoe UI',12), command=self.toggle_credentials_fields).grid(row=0, column=1, padx=5, pady=5)
        self.credentials_frame = ttk.Frame(home_frame)
        self.credentials_frame.grid(row=1, column=0, columnspan=2, pady=10)
        
        ttk.Label(self.credentials_frame, text="Host").grid(row=0, column=0, sticky='e', padx=(0,5), pady=5)
        self.host_name = ttk.Entry(self.credentials_frame)
        self.host_name.grid(row=0, column=1, padx=5, pady=5)

        ttk.Label(self.credentials_frame, text="Username").grid(row=1, column=0, sticky='e', padx=(0,5), pady=5)
        self.user_name = ttk.Entry(self.credentials_frame)
        self.user_name.grid(row=1, column=1, padx=5, pady=5)

        ttk.Label(self.credentials_frame, text="Password").grid(row=2, column=0, sticky='e', padx=(0,5), pady=5)
        self.password = ttk.Entry(self.credentials_frame, show="*")
        self.password.grid(row=2, column=1, padx=5, pady=5)

        ttk.Label(self.credentials_frame, text="Database").grid(row=3, column=0, sticky='e', padx=(0,5), pady=5)
        self.database = ttk.Entry(self.credentials_frame)
        self.database.grid(row=3, column=1, padx=5, pady=5)

        ttk.Button(home_frame, text="Login", command=self.submit_credentials).grid(row=2,column=0,columnspan=2,pady=10)
    ...
    def submit_credentials(self):
        ...
        if self.conn:
            ...
            self.update_menu.update_credentials(self.conn, self.cursor, self.credentials)
            self.config(menu=self.menubar)
            self.create_statusbar()
            self.create_toolbar()
            messagebox.showinfo("Connection Successful", "Database connection established successfully.")
        else:
            messagebox.showerror("Connection Failed", "Failed to establish database connection.")
    ...
    def connect_to_database(self, credentials):
        try:
            ...
            cursor.execute("CREATE DATABASE IF NOT EXISTS stock_portfolio")
            conn.commit()
    ...
    def create_toolbar(self):
        ...
        relogin_button = tk.Button(self.toolbar, text="Re-login", command=self.show_login_page)
