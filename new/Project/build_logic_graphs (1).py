...
index_to_table_df = pd.read_csv(r"index_to_table_mapping_nse.csv")
...
class BuildPortfolioMenu(tk.Menu):
    def __init__(self, master):
        ...
        self.add_command(
            label="Enter Risk Free Rate of Return", command=self.risk_free_return
        )
    ...
    def initial_inputs(self):
        ...
        self.scrollbar = ttk.Scrollbar(
            nse_indices_frame, orient=tk.VERTICAL, command=self.canvas.yview
        )
        ...
        self.canvas.create_window(
            (0, 0), window=self.nse_indices_inner_frame, anchor="nw"
        )
        ...
        for i, text in enumerate(bse_indices):
            ...
            if text == "BSE ALL":
                var.trace(
                    "w",
                    lambda *args, v=var: self.handle_all_indices(
                        v, self.bse_indices_vars, self.bse_indices_widgets
                    ),
                )
        ...
        for i, text in enumerate(nse_indices):
            ...
            widget = ttk.Checkbutton(
                self.nse_indices_inner_frame, text=text, variable=var
            )
            widget.pack(anchor="w", padx=5, pady=2)
            ...
            if text == "NSE ALL":
                var.trace(
                    "w",
                    lambda *args, v=var: self.handle_all_indices(
                        v, self.nse_indices_vars, self.nse_indices_widgets
                    ),
                )

        # Add widgets to the commodities frame
        self.gold_var = tk.BooleanVar()
        ttk.Checkbutton(commodities_frame, text="Gold", variable=self.gold_var).pack(
        ...
        for i, text in enumerate(forex):
            ttk.Checkbutton(forex_frame, text=text, variable=self.forex_vars[i]).pack(
                anchor="w", padx=5, pady=2
            )

        # Add widgets to the time period frame
        ttk.Label(time_period_frame, text="Start Date:").grid(
        ...
        ttk.Label(time_period_frame, text="End Date:").grid(
        ...
        ttk.Label(time_period_frame, text="Interval:").grid(
        ...
        self.next_button = ttk.Button(
            time_period_frame, text="Next >", command=self.execute
        )
    ...
    def calculations(self):
        STOCK_INDEXES = [
            self.nse_indices_widgets[i].cget("text")
            for i, var in enumerate(self.nse_indices_vars)
            if var.get()
        ]
        ...
        try:
            ...
            messagebox.showerror(
                "Invalid Input", "Please enter a valid integer for the interval."
            )
            return
        ...
        temp = pd.read_sql_query(
            f"select * from {self.DATABASE_NAME}.mstr_{STOCK_EXCHANGE}_indexes_eod WHERE Index_name = '{INDEX_TO_REGRESS_WITH_NSE}' AND Date BETWEEN '{self.START_DATE}' AND '{self.END_DATE}' ",
            self.conn,
        )
        temp = temp[["Index_name", "Date", "Close"]]
        # renaming the column close to close market as to not confuse it with close of stocks
        temp.rename(columns={"Close": "close_market"}, inplace=True)
        ...
        for key in self.df_512:
            self.df_512[key] = self.df_512[key].iloc[
                :: INTERVAL + 1
            ]  # logic to implement interval
            self.df_512[key] = self.df_512[key].sort_values(by="Date")
            self.df_512[key]["Previous_Close_stock"] = self.df_512[key][
                "Close_stock"
            ].shift(1)
            self.df_512[key]["Previous_Close_market"] = self.df_512[key][
                "Close_market_index"
            ].shift(1)
            self.df_512[key].dropna(inplace=True)

            self.df_512[key]["Return_Stock"] = (
                self.df_512[key]["Close_stock"]
                - self.df_512[key]["Previous_Close_stock"]
            ) / self.df_512[key]["Previous_Close_stock"]
            self.df_512[key]["Return_market"] = (
                self.df_512[key]["Close_market_index"]
                - self.df_512[key]["Previous_Close_market"]
            ) / self.df_512[key]["Previous_Close_market"]

            self.df_512[key]["returns_percentage"] = (
                self.df_512[key]["Return_Stock"] * 100
            )
            self.df_512[key]["returns_market_percentage"] = (
                self.df_512[key]["Return_market"] * 100
            )
        ...
        for stock in self.df_512:
            ...
            stock_return = (
                self.df_512[stock]["returns_percentage"].to_numpy().reshape(-1, 1)
            )
            market_return = (
                self.df_512[stock]["returns_market_percentage"]
                .to_numpy()
                .reshape(-1, 1)
            )
            ...
            mean_returns_percentage_stock = self.df_512[stock][
                "returns_percentage"
            ].mean()

            co_relation = self.df_512[stock]["Close_stock"].corr(
                self.df_512[stock]["Close_market_index"]
            )
            ...
            results.append(
                {
                    ...
                    "annualized_returns_percentage (mean_daily_returns*365)": mean_returns_percentage_stock
                    * 365,
                    "Alpha": ALPHA,
                    "BETA": BETA,
    ...
    def risk_free_return(self):
        ...
        # Risk Free Rate
        ttk.Label(
            input_frame, text="Risk Free Rate Of Return (%)", font=("Arial", 12)
        ...
        # Rate of Return
        ttk.Label(input_frame, text="Rate of Return", font=("Arial", 12)).grid(
            row=1, column=0, padx=10, pady=10, sticky="e"
        )
        self.rate_of_return_combobox = ttk.Combobox(
            input_frame,
            values=["Average Return", "Expected Return"],
            font=("Arial", 12),
            width=20,
        )
        ...
        # Ticker
        ttk.Label(input_frame, text="Ticker", font=("Arial", 12)).grid(
        ...
        self.delete_button = ttk.Button(
            button_frame, text="Delete", command=self.delete_fun, style="my.TButton"
        )
        self.delete_button.pack(side=tk.LEFT, padx=(10, 5))

        self.next_button = ttk.Button(
            button_frame, text="Next >", command=self.grid_view, style="my.TButton"
        )
        ...
        try:
            ...
            print("INVALID INPUT")
        self.table_1 = self.table_1[
            (self.table_1["returns_percentage"] > RATE_RISK_FREE_RETURN_NSE)
            & (self.table_1["BETA"] > 0)
        ]
        self.table_1 = self.table_1.sort_values(
            by="returns_percentage", ascending=False
        )
        # adding column excessive returns per beta
        self.table_1["excess returns"] = (
            self.table_1["returns_percentage"] - RATE_RISK_FREE_RETURN_NSE
        )
        self.table_1["excess returns to beta"] = (
            self.table_1["returns_percentage"] - (RATE_RISK_FREE_RETURN_NSE)
        ) / self.table_1["BETA"]

        # display the below dataframe in window_2_table_1
        window_2_table_1 = self.table_1[
            [
                "NSETICKER",
                "returns_percentage",
                "excess returns",
                "BETA",
                "unsystematic_risk",
                "excess returns to beta",
            ]
        ].copy(deep=True)

        cutoff_table = self.table_1[
            [
                "NSETICKER",
                "excess returns to beta",
                "BETA",
        ...
        ].sort_values(by="excess returns to beta", ascending=False)
        cutoff_table["(BETA^2)/(UNSYSTEMATIC_RISK)"] = (cutoff_table["BETA"] ** 2) / (
            cutoff_table["unsystematic_risk"]
        )
        cutoff_table["cummulative_intermediate"] = cutoff_table[
            "(BETA^2)/(UNSYSTEMATIC_RISK)"
        ].cumsum()
        cutoff_table["treynor_ratio_times_A"] = (
            cutoff_table["excess returns to beta"]
            * cutoff_table["(BETA^2)/(UNSYSTEMATIC_RISK)"]
        )
        cutoff_table["cumulative_treynor"] = cutoff_table[
            "treynor_ratio_times_A"
        ].cumsum()
        cutoff_table["cutoff_point"] = (
            cutoff_table["variance_market"] * cutoff_table["cumulative_treynor"]
        ) / (
            1
            + (
                cutoff_table["variance_market"]
                * cutoff_table["cummulative_intermediate"]
            )
        )
        ...
        weighted_table["excess_trenor"] = (
            weighted_table["excess returns to beta"] - weighted_table["cutoff_max"]
        )
        weighted_table["Z_i"] = weighted_table["excess_trenor"] * (
            weighted_table["BETA"] / weighted_table["unsystematic_risk"]
        )

        # display this table in window_2_table_3
        window_2_table_3 = weighted_table[
            ["NSETICKER", "cutoff_point", "cutoff_max", "Z_i"]
        ].copy(deep=True)
        ...
        weighted_table["weighted_return"] = (
            weighted_table["returns_percentage"] * weighted_table["weight_percentage"]
        )

        # display the below table in window_2_table_4
        window_2_table_4 = weighted_table[
            ["NSETICKER", "returns_percentage", "weight_percentage", "weighted_return"]
        ].copy(deep=True)
    ...
    def grid_view(self):
        ...
        tables = [
            self.window_2_table_1,
            self.window_2_table_2,
            self.window_2_table_3,
            self.window_2_table_4,
        ]
        ...
        for frame, table, title in zip(frames, tables, titles):
            ...
            compare_button = ttk.Button(
                button_frame,
                text="Compare Portfolio Returns with Market",
    ...
    def delete_fun(self):
        ...
        confirm = messagebox.askyesno(
            "Confirm Deletion", "Are you sure you want to delete the selected rows?"
        )
        if confirm:
            # Get the BSETICKER values for the selected items
            selected_nsetickers = [
                self.tree.set(item, "NSETICKER") for item in selected_items
            ]

            # Remove the selected tickers from table_1
            self.table_1 = self.table_1[
                ~self.table_1["NSETICKER"].isin(selected_nsetickers)
            ]
            ...
            messagebox.showinfo(
                "Deletion Successful", f"{len(selected_items)} row(s) deleted."
            )

    def comparison(self):
        portfolio_stocks_and_weights = {
            row["NSETICKER"]: row["weight_percentage"]
            for index, row in self.window_2_table_4.iterrows()
        }
        ...
        def get_stock_value(
            ticker: str, date: str, db_cursor=self.cursor, max_days: int = 365
        ):
        ...
        def make_portfolio(
            portfolio_with_weights: dict,
            start_date: str = START_DATE,
            db_cursor=self.cursor,
            portfolio_amount: int = 100000,
        ):
            portfolio = dict()
            for stock in portfolio_with_weights:
                amount_invested_in_stock = portfolio_amount * (
                    portfolio_with_weights[stock] / 100
                )
                stock_value_on_start_date = get_stock_value(
                    stock, start_date, db_cursor=db_cursor
                )
                if stock_value_on_start_date is None:
                    print(stock, start_date, stock_value_on_start_date)
                    continue
                no_of_shares_of_stock = int(
                    amount_invested_in_stock / stock_value_on_start_date
                )
        ...
        def get_portfolio_value(date: str, portfolio_dict, db_cursor=self.cursor):
            ...
            for stock in portfolio_dict:
                stock_value = get_stock_value(
                    date=date, ticker=stock, db_cursor=db_cursor
                )
        ...
        def get_index_value(
            date: str,
            index_name: str = INDEX_TO_REGRESS_WITH_NSE,
            db_cursor=self.cursor,
            max_days: int = 365,
        ):
            DATE = datetime.strptime(date, "%Y-%m-%d").date()
        ...
        def calculate_portfolio_returns(
            ...
            value_on_starting = get_portfolio_value(
                start_date, db_cursor=db_cursor, portfolio_dict=portfolio_dict
            )
            value_on_ending = get_portfolio_value(
                end_date, db_cursor=db_cursor, portfolio_dict=portfolio_dict
            )
            returns_percentage = (
                (value_on_ending - value_on_starting) / (value_on_starting)
            ) * 100
        ...
        def get_data(
            ...
            for date in dates:
                ...
                datewise_index_returns.append(
                    calculate_index_returns(end_date=date, db_cursor=db_cursor)
                )
        ...
        portfolio_dict = make_portfolio(
            portfolio_stocks_and_weights, START_DATE, self.cursor
        )
        dates, datewise_returns, datewise_index_returns = get_data(
            START_DATE, self.END_DATE.strftime("%Y-%m-%d"), portfolio_dict, self.cursor
        )
        print("DATES.............", dates)
        ...
        def plot_results(
            ...
            range: tuple = (-10, 10),
            precision: int = 100,
            no_of_bins_hist: int = 25,
        ):
            fig, axes = plt.subplots(nrows=1, ncols=2, figsize=(10, 6))

            axes[0].hist(
                self.df_512[ticker]["returns_percentage"],
                bins=no_of_bins_hist,
                edgecolor="black",
            )
        ...
        def display_analysis():
            ...
            stock_dropdown = ttk.Combobox(
                analysis_window, textvariable=selected_stock, values=stock_list
            )
            ...
            def run_analysis():
                ...
                plot_results(
                    ...
                    no_of_bins_hist=bins,
                )

            analyze_button = ttk.Button(
                analysis_window, text="Analyze", command=run_analysis
            )
        ...
        analyze_button = ttk.Button(
            button_frame, text="analyze the portfolio", command=display_analysis
        )
        analyze_button.pack()
