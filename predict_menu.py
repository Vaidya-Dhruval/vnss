...
class predict(tk.Menu):
    ...
    def open_prediction_window(self):
        ...
        self.prediction_window.geometry("800x600")  # Increased width to accommodate multiple columns
        ...
        ttk.Button(self.prediction_window, text="Start Prediction", command=self.start_prediction).pack(pady=10)

        # Add a progress bar
        self.progress_bar = ttk.Progressbar(self.prediction_window, orient="horizontal", length=300, mode="determinate")
    ...
    def start_prediction(self):
        ...
        if not selected_tickers:
            tk.messagebox.showwarning("No Selection", "Please select at least one stock.")
            return
        ...
        for i, ticker in enumerate(selected_tickers):
            ...
            model = self.create_lstm_model(input_shape, num_lstm_nodes, num_lstm_layers, dropout_rate)
            model.compile(optimizer=optimizer, loss=loss_function, metrics=['mae', 'mse', 'mean_absolute_percentage_error'])
            
            checkpointer = ModelCheckpoint(filepath=f'models/{ticker}_best.keras', monitor='val_loss', verbose=0, save_best_only=True)
            ...
            class EpochLogger(keras.callbacks.Callback):
                ...
                def on_epoch_end(self, epoch, logs=None):
                    self.result_text.insert(tk.END, f"Epoch {epoch+1}/{num_epochs} completed\n")
                    self.result_text.see(tk.END)
                    self.result_text.update()
            ...
            last_data = ticker_data[['Open', 'High', 'Low', 'Close', 'Volume']].values[-historical_data_length:]
            predicted_price = self.predict_future_prices(model, last_data, historical_data_length, scaler, forecast_horizon)
            
            self.result_text.insert(tk.END, f"Predicted price for {ticker}: {predicted_price[0]:.2f}\n")
            self.result_text.see(tk.END)
            self.result_text.update()
    ...
    def create_lstm_model(self, input_shape, num_lstm_nodes, num_lstm_layers, dropout_rate):
        model = Sequential()
    ...
    def predict_future_prices(self, model, data, historical_data_length, scaler, forecast_horizon):
        ...
        predicted_price = scaler.inverse_transform(np.hstack((np.zeros((predicted_price.shape[0], 3)), predicted_price, np.zeros((predicted_price.shape[0], 1)))))[:, 3]
