...
import numpy as np
import pandas as pd
from eqldata import (DataClient, generate_auth_token, get_EOD,
                     get_instrument_list)
...
class CredentialDialog(tk.Toplevel):
    def __init__(self, parent, callback):
        ...
        ttk.Label(main_frame, text="Select Credential Type:", font=('Segoe UI', 12, 'bold')).pack(anchor='w', pady=(0, 10))
        
        ttk.Radiobutton(main_frame, text="Default Credentials", variable=self.cred_type, 
                       value="default", command=self.toggle_fields).pack(anchor='w', pady=5)
        ttk.Radiobutton(main_frame, text="Custom Credentials", variable=self.cred_type, 
                       value="custom", command=self.toggle_fields).pack(anchor='w', pady=5)
        ...
        self.username_entry = ttk.Entry(self.cred_frame, textvariable=self.username_var, width=40)
        ...
        self.password_entry = ttk.Entry(self.cred_frame, textvariable=self.password_var, show="*", width=40)
        self.password_entry.pack(anchor='w', pady=(0, 10))
        
        # Buttons frame
        btn_frame = ttk.Frame(self)
        btn_frame.pack(fill=tk.X, padx=20, pady=(0, 20), anchor='center')  # Center the button frame
...
class update_db(tk.Menu):
    ...
    def update_database(self):
        ...
        engine = create_engine(f"mysql+pymysql://{USER_NAME}:{PASSWORD}@{HOST_ADDRESS}/{self.DATABASE_NAME}")
        try:
            self.cursor.execute("USE updated_stocks")
        except:
            self.cursor.execute("CREATE DATABASE updated_stocks")
            self.cursor.execute("USE updated_stocks")
        
        query = "SELECT MAX(Date) FROM mstr_nse_indexes_eod WHERE Index_name = 'NIFTY_50'"
        last_date_df = pd.read_sql(query, con=self.conn)
        self.log_to_frame(str(last_date_df))
        ...
        try:
            ...
            if "NSEIDX" not in list_of_securities:
                self.log_to_frame("No NSE indexes found in instrument list. Aborting update.")
                messagebox.showerror("Error", "No NSE indexes found in instrument list.")
                return
        ...
        for i, current_date in enumerate(date_list):
            try:
                ...
                if existing_data.iloc[0, 0] > 0:
                    self.log_to_frame(f"Data for {current_date} already exists, skipping.")
                    continue
                ...
                if result == "Error":
                    self.log_to_frame(f"Couldn't get data for: {current_date} because of holidays")
                    continue
                if result == "Too Many Requests":
                    self.log_to_frame(f"Couldn't get data for date: {current_date} due to server overload")
